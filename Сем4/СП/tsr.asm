...
;
; Автор:
;  МГТУ им. Н.Э. Баумана, ИУ5-44Б, 2025 г.
...
code segment	'code'
	...
	; данные
	ignoredChars 				DB	'АБВГДЕЁЖЗИЙКЛМНОПХЦЧШЩХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопХЦЧШЩхцчшщъыьэюя'	;@ список игнорируемых символов
	ignoredLength 				equ	$-ignoredChars				; длина строки ignoredChars
	replaceWith 				DB  'F*DULT**PBQRKVYJG{WXIO*WXIO*SM**Zf*dult**pbqrkvyjg{WXIO*wxio*sm**z'
	ignoreEnabled 				DB	0							; флаг функции игнорирования ввода
	translateFrom 				DB	'{WXIO'						;@ символы для замены (ХЦЧШЩ на англ. раскладке)
	translateTo 				DB	'ХЦЧШЩ'						;@ символы на которые будет идти замена
	...
	charToCursiveIndex 			DB 'Э'							;@ символ для замены
	...
	signatureLine1				DB	179, ' Чернев Николай Андреевич ', 179
	Line1_length 					equ	$-signatureLine1
	signatureLine2				DB	179, '          ИУ5-44Б         ', 179
	Line2_length 					equ	$-signatureLine2
	signatureLine3				DB	179, '        Вариант #22       ', 179
	...
	helpMsg DB '>tsr.com [/?]', 10, 13
			...
			DB '  F2  - вывод ФИО и группы через 5 секунд сверху экрана', 10, 13
			DB '  F3  - включение/отключение курсивного вывода русского символа Э', 10, 13
			DB '  F4  - включение/отключение частичной русификации клавиатуры({WXIO -> ХЦЧШЩ)', 10, 13
			DB '  F5  - включение/отключение режима запрета ввода русских букв', 10, 13
